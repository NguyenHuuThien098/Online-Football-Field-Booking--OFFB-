...
const addField = async (req, res) => {
    const { name, location, type, price, image, ownerId, contactNumber, operatingStart, operatingEnd } = req.body;
    ...
    // Kiểm tra giờ hoạt động hợp lệ
    if (!operatingStart || !operatingEnd) {
        return res.status(400).json({ message: 'Vui lòng nhập giờ bắt đầu và giờ kết thúc cho sân.' });
    }
    ...
    try {
        ...
        console.error('Error when adding field:', error);
